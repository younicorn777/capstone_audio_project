...
INPUT_CSV = os.path.join(BASE_DIR, "results", "ml", "yamnet_embeddings.csv")

RESULT_DIR = os.path.join(BASE_DIR, "results", "ml", "visualization")
os.makedirs(RESULT_DIR, exist_ok=True)

PCA_PLOT_PATH = os.path.join(RESULT_DIR, "yamnet_embedding_pca.png")
TSNE_PLOT_PATH = os.path.join(RESULT_DIR, "yamnet_embedding_tsne.png")
PCA_CSV_PATH = os.path.join(RESULT_DIR, "yamnet_embedding_pca_coordinates.csv")
TSNE_CSV_PATH = os.path.join(RESULT_DIR, "yamnet_embedding_tsne_coordinates.csv")
...
def run_pca(X_scaled, labels, files):
    print("PCA 시각화 생성 중...")
    ...
    plot_2d_embedding(
        coords=coords,
        labels=labels,
        title=f"YAMNet Embedding PCA\nExplained Variance: {np.sum(explained):.2%}",
    ...
    print(f"PCA plot 저장: {PCA_PLOT_PATH}")
    print(f"PCA 좌표 CSV 저장: {PCA_CSV_PATH}")
...
def run_tsne(X_scaled, labels, files):
    ...
    # 데이터 수가 작을 때 perplexity가 너무 크면 오류가 나므로 자동 조정
...
def main():
    ...
    run_pca(X_scaled, labels, files)
    run_tsne(X_scaled, labels, files)

    print("\n" + "-" * 70)
    print("YAMNet embedding 시각화 완료")
