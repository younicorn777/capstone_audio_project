...
PLOT_DIR = os.path.join(BASE_DIR, "results", "plots", "moving_average")
...
def get_clean_file_from_noisy(noisy_file):
    ...
    noisy_clean02_dog.wav → clean_02.wav
    """
    # noisy_clean01_air.wav
    parts = noisy_file.replace(".wav", "").split("_")
...
def plot_comparison(clean, noisy, ma, title, output_path):
    min_len = min(len(clean), len(noisy), len(ma))
    ...
    plt.subplot(3, 1, 3)
    plt.plot(t, ma)
    plt.title("Moving Average Result")
    plt.xlabel("Time (sec)")
    plt.ylabel("Amplitude")
    plt.ylim(-1, 1)
...
def main():
    ...
    for noisy_file in noisy_files:
        ...
        plot_name = noisy_file.replace(".wav", "_waveform.png")
        output_path = os.path.join(PLOT_DIR, plot_name)

        title = noisy_file.replace(".wav", "")
        plot_comparison(clean, noisy, ma, title, output_path)

        print(f"[저장 완료] {plot_name}")

    print("-" * 50)
    print("파형 비교 이미지 생성 완료!")
